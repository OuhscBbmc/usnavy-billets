...
### Notes 
1. The current demonstration covers `r nrow(ds_officer_roster)` officers, `r nrow(ds_command_roster)` unique commands, and `r sum(ds_command_roster$billet_count_max)` total possible billets.  In the specialty of emergency medicine, the term "command" and "command" can almost be used interchangably.  Almost all commands are located in a single command, and each command/command can have multiple billets.
...
To walk through an example from the command's perspective, look at the fifth row in the first table.  The values represent how command ```r ds_command_transition$command_id[5]``` ranked the `r nrow(ds_officer_roster)` officers.  The command's initial four officers (i.e., ```r paste(colnames(ds_command_transition)[2:5], collapse=", ")```) were ranked `r as.data.frame(ds_command_transition[5, 2:4])`, and `r as.data.frame(ds_command_transition[5, 5])`.

To walk through an example from the officer's perspective, look at the second row in the second table.    The values represent how officer ```r ds_officer_transition$officer_id[2]``` ranked the `r nrow(ds_command_roster)` commands.  The officer's initial three commands (i.e., ```r paste(colnames(ds_officer_transition)[2:4], collapse=", ")```) were ranked `r as.data.frame(ds_officer_transition[2, 2:3])`, and `r as.data.frame(ds_officer_transition[2, 4])`.
